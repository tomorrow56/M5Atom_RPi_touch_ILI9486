...
#define SELECT_L_PIN    22 // LEFT eye chip select pin
#define SELECT_R_PIN    22 // RIGHT eye chip select pin
...
#define WINK_R_PIN     39 // Pin for RIGHT eye wink button
...
void drawEye( // Renders one eye.  Inputs must be pre-clipped & valid.
  ...
  uint8_t  e,       // Eye array index; 0 or 1 for left/right�F0/1 = left/right
  uint32_t iScale,  // Scale factor for iris
  uint32_t  scleraX, // First pixel X offset into sclera image�FCenter of right and left
  uint32_t  scleraY, // First pixel Y offset into sclera image�FUpper and lower centers
  uint32_t  uT,      // Upper eyelid threshold value�FUp and down
  ...
  eye[e].tft.setAddrWindow(81 + 190 * e, 10, 128, 128); //set eye window
  ...
  for(screenY=0; screenY<SCREEN_HEIGHT; screenY++, scleraY++, irisY++) {
    ...
    for(screenX=0; screenX<SCREEN_WIDTH; screenX++, scleraX++, irisX++) {
      ...
      if((pgm_read_byte(lower + screenY * SCREEN_WIDTH + screenX_) <= lT) ||
         (pgm_read_byte(upper + screenY * SCREEN_WIDTH + screenX_) <= uT)) {             // Covered by eyelid
        p = 0;
